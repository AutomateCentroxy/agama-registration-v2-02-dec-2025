...
public class JansUserRegistration extends NewUserRegistration {
    ...
    private int parseInt(String value, String keyName) {
        if (value == null || value.trim().isEmpty()) {
            throw new RuntimeException("Missing required config key: " + keyName);
        }
        try {
            return Integer.parseInt(value.trim());
        } catch (Exception e) {
            throw new RuntimeException("Invalid integer value for " + keyName + ": " + value);
        }
    }

    private long parseLong(String value, String keyName) {
        if (value == null || value.trim().isEmpty()) {
            throw new RuntimeException("Missing required config key: " + keyName);
        }
        try {
            ...
            throw new RuntimeException("Invalid long value for " + keyName + ": " + value);
        }
    }

    private void initRateLimitConfig() {

        maxSmsOtpPerDay = parseInt(flowConfig.get("MAX_SMS_OTP_PER_DAY"), "MAX_SMS_OTP_PER_DAY");
        maxRegAttemptsPerDay = parseInt(flowConfig.get("MAX_REG_ATTEMPTS_PER_DAY"), "MAX_REG_ATTEMPTS_PER_DAY");
        timeWindowMs = parseLong(flowConfig.get("TIME_WINDOW_MS"), "TIME_WINDOW_MS");

        emailOtpLimit = parseInt(flowConfig.get("EMAIL_OTP_LIMIT"), "EMAIL_OTP_LIMIT");
        emailWindowMs = parseLong(flowConfig.get("EMAIL_WINDOW_MS"), "EMAIL_WINDOW_MS");
        emailBlockMs = parseLong(flowConfig.get("EMAIL_BLOCK_MS"), "EMAIL_BLOCK_MS");

        String whitelist = flowConfig.get("WHITELISTED_IPS");
        if (whitelist == null || whitelist.trim().isEmpty()) {
            throw new RuntimeException("Missing required config key: WHITELISTED_IPS");
        }

        whitelistedIps = Arrays.stream(whitelist.split(","))
                .map(String::trim)
                .filter(s -> !s.isEmpty())
